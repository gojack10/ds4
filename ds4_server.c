...
static bool parse_generated_message_for_response_for_syntax(
    ...
    bool parsed_ok = parse_generated_message_ex_for_syntax(
...
static DS4_SERVER_MAYBE_UNUSED bool parse_generated_message_for_response(
    ...
    return parse_generated_message_for_response_for_syntax(
        ...
        require_thinking_closed, orders, finish_io, err, errlen, content_out,
        reasoning_out, calls, recovered_out, coerced_summary_out,
        rejected_reason_out);
}
...
static void server_activity_set_prefill(server_slot *slot, int processed,
    ...
    if (!slot) return;
    pthread_mutex_lock(&slot->activity_mu);
    slot->activity.phase = SERVER_ACTIVITY_PREFILL;
    slot->activity.processed = processed;
    slot->activity.total = total;
    slot->activity.generated_tokens = 0;
    slot->activity.tok_s = tok_s;
    slot->activity.eta = eta;
    pthread_mutex_unlock(&slot->activity_mu);
}
...
static void server_activity_set_decode(server_slot *slot, int generated_tokens,
    ...
    if (!slot) return;
    pthread_mutex_lock(&slot->activity_mu);
    slot->activity.phase = SERVER_ACTIVITY_DECODE;
    slot->activity.processed = 0;
    slot->activity.total = 0;
    slot->activity.generated_tokens = generated_tokens;
    slot->activity.tok_s = tok_s;
    slot->activity.eta = 0.0;
    pthread_mutex_unlock(&slot->activity_mu);
}
...
static void test_openai_tool_stream_sends_partial_arguments(void) {
    ...
    char *parsed_reasoning = NULL;
    tool_calls calls = {0};
    TEST_ASSERT(parse_generated_message_ex(raw_complete, false, NULL, NULL,
...
static void test_openai_glm_tool_stream_suppresses_raw_tool_call(void) {
    ...
    TEST_ASSERT(parse_generated_message_ex_for_syntax(
        SERVER_MODEL_SYNTAX_GLM, raw, false, &r.tool_orders, NULL,
        &parsed_content, &parsed_reasoning, &calls));
    TEST_ASSERT(calls.len == 1);
...
static void test_parse_glm_tool_call_message(void) {
    ...
    TEST_ASSERT(parse_generated_message_ex_for_syntax(
        SERVER_MODEL_SYNTAX_GLM, generated, true, NULL, NULL,
...
static void test_dsml_repair_produces_parseable_calls(void) {
    ...
    /* === TEST 13: Real DSML after thinking still repairs normally === */
    {
        ...
        TEST_ASSERT(parse_generated_message_ex(repaired.ptr, true, NULL, NULL,
                                               &content, &reasoning, &calls));
        TEST_ASSERT(calls.len == 1);
        TEST_ASSERT(calls.v[0].name && !strcmp(calls.v[0].name, "bash"));
...
static void test_glm_tool_checkpoint_suffix_is_canonical(void) {
    ...
    tool_calls calls = {0};
    TEST_ASSERT(parse_generated_message_ex_for_syntax(
        SERVER_MODEL_SYNTAX_GLM, generated, false, NULL, NULL,
        &content, &reasoning, &calls));
    TEST_ASSERT(calls.len == 1);
